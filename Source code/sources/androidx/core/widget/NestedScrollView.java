        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case KeyCodes.KEYCODE_SPACE /*{ENCODED_INT: 62}*/:
            ...
            case 2:
                ...
                if (activePointerIndex != -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        dispatchNestedScroll(0, scrolledDeltaY, 0, deltaY - scrolledDeltaY, this.mScrollOffset, 0, this.mScrollConsumed);
                        this.mLastMotionY -= this.mScrollOffset[1];
                        this.mNestedYOffset += this.mScrollOffset[1];
                        if (canOverscroll) {
                            int deltaY2 = deltaY - this.mScrollConsumed[1];
                            ensureGlows();
                            int pulledToY = oldY + deltaY2;
                            if (pulledToY < 0) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowTop, ((float) deltaY2) / ((float) getHeight()), ev.getX(activePointerIndex) / ((float) getWidth()));
                                if (!this.mEdgeGlowBottom.isFinished()) {
                                    this.mEdgeGlowBottom.onRelease();
                                }
                            } else if (pulledToY > range) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowBottom, ((float) deltaY2) / ((float) getHeight()), 1.0f - (ev.getX(activePointerIndex) / ((float) getWidth())));
